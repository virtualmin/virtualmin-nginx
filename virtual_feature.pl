...
my $server = &lock_domain_server($d);
...
foreach my $r (@rewrites) {
	...
	if (!$redirect && $r->{'words'}->[2] &&
		...
		# Special case for / which excludes .well-known, in either the
		# older unescaped or the current escaped dot form
		$redirect = { 'path' => $1,
			      'dest' => $r->{'words'}->[1],
			      'object' => $r,
			    };
